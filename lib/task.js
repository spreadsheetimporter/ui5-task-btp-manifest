...
module.exports = async function ({ log, workspace, dependencies, taskUtil, options }) {
	...
	const hasSpreadsheetImporter =
		packageJson.dependencies?.["ui5-cc-spreadsheetimporter"] ||
		packageJson.devDependencies?.["ui5-cc-spreadsheetimporter"];
	if (!hasSpreadsheetImporter) {
		isDebug && log.info("ui5-cc-spreadsheetimporter not found in dependencies. Skipping manifest change.");
		return;
	}
	const spreadsheetImporterVersionPoint = hasSpreadsheetImporter;
	// replace all "." with "_" from hasSpreadsheetImporter
	const spreadsheetimporterversion = spreadsheetImporterVersionPoint.replace(/\./g, "_");
	...
	// if spreadsheetImporterVersionPoint less than "0.34.0", then use customControl and spreadsheetImporter
	const useCustomControl = compareVersions(spreadsheetImporterVersionPoint, "0.34.0") < 0;
	const pathSegment = useCustomControl ? "customControl/spreadsheetImporter" : "customcontrol/spreadsheetimporter";

	const spreadsheetImporterManifestPath = `/thirdparty/${pathSegment}/v${spreadsheetimporterversion}/manifest.json`;
	const workspaceSpreadsheetImporterManifestPath = `/resources/${taskUtil
		.getProject()
		.getNamespace()}${spreadsheetImporterManifestPath}`;

	let spreadsheetImporterManifestResource = await dependencies.byPath(spreadsheetImporterManifestPath);
	if (spreadsheetImporterManifestResource) {
		taskUtil.setTag(spreadsheetImporterManifestResource, taskUtil.STANDARD_TAGS.OmitFromBuildResult, true);
	}

	spreadsheetImporterManifestResource = createResource({
		path: workspaceSpreadsheetImporterManifestPath,
	...
	try {
		...
		isDebug &&
			log.info("Successfully updated ui5-cc-spreadsheetimporter manifest at:", workspaceSpreadsheetImporterManifestPath);
	} catch (error) {
